...
export class ChankuraOrderGateway implements Interfaces.IOrderEntryGateway {
...
export class ChankuraPositionGateway implements Interfaces.IPositionGateway {
...
class ChankuraGateway extends Interfaces.CombinedGateway {
    constructor(config: Config.IConfigProvider, pair: Models.CurrencyPair) {
        const minTick = config.GetNumber("ChankuraGatewayTick");
        super(
            new ChankuraMarketDataGateway(minTick), 
            new ChankuraOrderGateway(), 
            new ChankuraPositionGateway(pair), 
            new ChankuraGatewayDetails(minTick));
    }
}

export async function createChankura(config: Config.IConfigProvider, pair: Models.CurrencyPair) : Promise<Interfaces.CombinedGateway> {
    return new ChankuraGateway(config, pair);
}
